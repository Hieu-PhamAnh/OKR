...
const validateMiddleware = {
  ...
  checkValidIdUser: async (req, res, next) => {
    const { id_verify } = req.body;
    const { id } = req.params;
    // console.log(id_verify, " ", id);
    // console.log(typeof id_verify, typeof id);
    if (id_verify !== id) {
      return res.status(400).json({
        message: "id truyền trong params không khớp với id trong access token",
      });
    }
    next();
  },
